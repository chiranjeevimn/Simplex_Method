...
def objectiveFunction(coeffs):
    ...
    for coef in range(coeffs):
        print("Enter coeffecient of variable ",(coef+1)," in the objective equation")
        eq+=input()+','
    eq+='0'
...
def main():
    print("Enter total number of constraint.")
    ...
    for cons in range(constraints):
        eq=''
        for coef in range(coeffs):
            print("Enter coeffecient of variable ",(coef+1)," in the equation",(cons+1))
            eq+=input()+','
        print("Choose inequality. For <= Press 1, for >= Press 2")
        ineq=int(input())
        if ineq == 1:
            eq +='LTE,'
        elif ineq == 2:
    ...
    print("Press 1 for Maximazation Problem. \n Press 2 for Minimization problem")
    objType = int(input())
